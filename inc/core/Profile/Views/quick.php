			...
			<div class="card">
				<div class="card-body bg-gray-800 shadow b-r-15 d-flex p-0 justify-content-between">
					...
					<div class="flex-fill pt-5">
						<img src="<?php _ec( get_module_path( __DIR__, "Assets/img/favicon.png" ) )?>" class="w-99"> 
					</div>
					...
					<?php if (!empty($result)): ?>
						...
						<?php foreach ($result as $key => $value): ?>
							<?php if ($value['name']  == "Account manager" || $value['name']  == "File manager" || $value['name']  == "Schedules" ): ?>
								<a href="<?php _ec( base_url( $value["id"] ) )?>" class="d-block border b-r-15 p-20 miw-200 text-over-all bg-gray-100 me-4 text-center">
